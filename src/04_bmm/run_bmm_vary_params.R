...
runM2 <- function(trait){
    ...
    options(mc.cores = parallel::detectCores())
    rstan_options(auto_write = TRUE)
    fit2 <- stan(file = "src/04_bmm/models/model2_loglik.stan",  
            data = dat$dat,    
            chains = 4, warmup =200, iter = 600, cores = 4, refresh = 200)
    ...
    print("SAVING")
    rm(dat)
    save(fit2, file=sprintf("%s/m2/f_m2_%s_ll.RData", out_dir, trait))
}
...
runM2_alt <- function(trait){
    ...
    options(mc.cores = parallel::detectCores())
    rstan_options(auto_write = TRUE)
    fit2 <- stan(file = "src/04_bmm/models/model2_alt_loglik.stan",  
            data = dat$dat,    
            chains = 4, warmup =200, iter = 600, cores = 4, refresh = 200)
    ...
    print("SAVING")
    rm(dat)
    save(fit2, file=sprintf("%s/m2/f_alt_m2_%s_ll.RData", out_dir, trait))
}
