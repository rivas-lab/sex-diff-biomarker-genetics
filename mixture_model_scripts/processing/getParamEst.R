...
overallHeritability <- function(dat, fit){
    ...
    #h_f <- num_f/(num_f + sum(dat$dat$SE[,1]))
    #h_m <- num_m/(num_m + sum(dat$dat$SE[,2]))

    ### binary traits?
    # f <- function(p) { p*(1-p)/dnorm(pnorm(1-p))^2}
    # (f(p))*h_f, (f(p))*h_m
    #
...
list.input <- read.delim("multi_run_gwas_input.txt", header=FALSE)
list.input$trait.names <- sapply(list.input$V4, function(x) gsub(" ", "_", x))
files.present <- sapply(list.input$V1, function(trait)
    {my.file <- sprintf("%s/f_m1_%s_m%s_s%s.RData", DATA.FOLDER, trait, maf, se)
...
filt.input <- list.input[files.present,]
list.out <- lapply(1:nrow(filt.input), function(i) loadTrait(filt.input$V1[i], filt.input$trait.names[i]))
df <- do.call(rbind,list.out)
colnames(df) <- col.labels 
write.table(df, file=sprintf("%s/bf_data_df.txt", DATA.FOLDER))

#list.traits <- c("arm_fp", "leg_fp", "trunk_fp", "INI23099")