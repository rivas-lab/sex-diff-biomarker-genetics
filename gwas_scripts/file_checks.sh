...
GWAS_FOLDER=/scratch/PI/mrivas/users/erflynn/sex_div_gwas/results/
...
do
	...
	if [[ ! -s ${f_path} ]]; then echo "File for F c${CHR} is missing"; fi
	if [[ ! -s ${m_path} ]]; then echo "File for M c${CHR} is missing"; fi
