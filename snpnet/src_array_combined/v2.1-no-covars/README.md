...
```{bash}
for pop_name in combined onesex zerosex ; do
    sbatch -p mrivas --qos=high_p --job-name=snpnet_no-${pop_name} snpnet-no-covars.sh ${pop_name}
    sbatch -p mrivas --qos=high_p --job-name=snpnet_bc-${pop_name} snpnet-basic-covars.sh ${pop_name}
done
```

```{text}
Submitted batch job 66068043
Submitted batch job 66068044
Submitted batch job 66068046
Submitted batch job 66068054
Submitted batch job 66068056
Submitted batch job 66068058
```
